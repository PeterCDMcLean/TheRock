...
def _as_text(output: str | bytes | None) -> str:
    """Decode partial output captured on a timeout.

    `subprocess.run(..., text=True)` decodes only on the success path. When it
    raises TimeoutExpired, the partial output was joined from the raw byte
    chunks before the text wrapper saw them, so `.stdout` is `bytes` -- and
    printing that directly emits a single escaped b'...' repr, exactly in the
    wedged-emulator case this capture exists to make legible.
    """
    if output is None:
        return ""
...
def main() -> int:
    ...
    try:
        ...
        print(_as_text(timeout.stdout))
        print(_as_text(timeout.stderr), file=sys.stderr)
        print(
